...
ID3D11SamplerState				*sampler = nullptr; // can be reused for any other texture, but setup here for use with the bee texture
...
// Model
DCStripification*				model = nullptr;
#pragma endregion
...
#pragma endregion
	...
	basicTexturePipeline = new CGPipeline(device, "Resources\\Shaders\\basic_texture_vs.hlsl", nullptr, "Resources\\Shaders\\basic_texture_ps.hlsl", nullptr, defaultRSStage, defaultOMStage, &vsExtBytecode);

	// Setup models
	//model = new JStrip(device, vsExtBytecode, L"Resources\\Models\\dropship.gsf");
	model = new DCStripification(device, vsExtBytecode, L"Resources\\Models\\chair.obj", L"Resources\\Textures\\chair.jpg", 1);

	// Setup scene objects
	basicScene.push_back(new CGModelInstance(model, XMFLOAT3(0.0f, -10.0f, 0.0f), XMFLOAT3(0.0f, 0.0f, 0.0f)));
...
void renderScene(void)
{
	...
	// 2. Get camera transformation matrices for the current frame
	...
	// Setup constant buffers for the current frame
	cameraBuffer->viewProjMatrix = viewMatrix * projectionMatrix;
	XMVECTOR eyepos = cam->getCameraPos(); // XMVECTOR camera position
	XMStoreFloat3(&(cameraBuffer->eyePos), eyepos); // Convert to float3 position

	// Get time since previous frame
	...
	ID3D11Buffer* vsCBuffers[] = {camera_cbuffer, gameTime_cbuffer, worldTransform_cbuffer}; // 0, 1, 2
	ID3D11Buffer* psCBuffers[] = {camera_cbuffer}; // 0

	context->VSSetConstantBuffers(0, 3, vsCBuffers);
	context->PSSetConstantBuffers(0, 1, psCBuffers);

	// Apply basic texture pipeline
	basicTexturePipeline->applyPipeline(context);

	// Setup resources -> Encapsulate
	...
	// 4. Present current frame to the screen - swap backbuffer
